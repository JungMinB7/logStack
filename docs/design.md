...
| 요청 본문 ≤ 4MB | 배치 상한 500건/3MB(소프트). 서버는 JSON 본문과 Content-Length가 있는 모든 요청에 4MB를 강제(413)한다. Content-Length 없는 청크형 비-JSON 전송은 본문을 버퍼링하지 않고 400으로 거부하며, 스트림 수준 크기 제한은 프록시가 담당한다 (§16) |
...
- INGEST_API_KEY와 ADMIN_API_KEY는 동일 값이면 부팅을 거부한다(역할 분리 —
  적재 키로 지표 조회가 가능해지는 설정 오류 차단)
...
롤백 보장의 주체는 DB 수준 timeout(statement_timeout 5초, 트랜잭션 8초)이며,
HTTP 데드라인 10초는 응답 절단만 수행한다 — 절단 후 커밋될 수 있는 한계와
그 안전성 근거는 §16에 기술한다.
...
- 서버는 JSON 본문과 Content-Length가 있는 모든 요청에 4MB를 강제한다(413).
  Content-Length 없는 청크형(Transfer-Encoding: chunked) 비-JSON 전송은
  본문을 버퍼링하지 않고 400 MALFORMED_REQUEST로 안전 거부하며(저장·메모리
  압박 없음), 스트림 수준 크기 제한은 리버스 프록시(§2.3)가 담당한다.
