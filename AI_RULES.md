...
## 인프라 규칙 (AWS 고도화)
28. 에이전트는 terraform apply·destroy를 실행하지 않는다. plan까지만. 실행은 사람.
29. 시크릿은 SSM Parameter Store(SecureString)에만. 코드·상태·로그에 평문 금지.
30. 인바운드 0.0.0.0/0 금지. SG는 plan-aws.md §1 체인만 허용.
31. DB 인스턴스는 퍼블릭 IP 없음 + 인터넷 라우트 없는 서브넷.
32. 전송측 앱은 design.md §4를 그대로 구현한다: 자체 스로틀 60req/분,
    응답 본문 파싱 후 ACK, 429 Retry-After + jitter 준수. 서버 429는 최후 방어선.
33. AWS 실측 수치는 실제 실행 결과만 기록한다 (기존 §10.7 정직성 원칙 동일 적용).